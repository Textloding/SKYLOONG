...
void ConvertYUV420SPToBGR(unsigned char *yData, unsigned char *vData, unsigned char *uData, unsigned char *Dst, int ImageWidth, int ImageHeight)
{
    ...
    for (int i = 0; i < ImageHeight; i++)
    {
        for (int j = 0; j < ImageWidth; j++)
        {
            ...
            rgb565 = (clamp(*yData + rdif) >> 3) << 11;
            rgb565 |= (clamp(*yData - invgdif) >> 2) << 5;
            rgb565 |= (clamp(*yData + bdif) >> 3);
            *(Dst++) = (rgb565 >> 8) & 0xFF;
            *(Dst++) = rgb565 & 0xFF;
            ++yData;
        }
...
void VideoPlayer::play(FILE *f)
{
    ...
    do
    {
        last_millis = millis();
        plm_decode(plm, 1);
        last_millis = 980 - (millis() - last_millis);
        if (last_millis > 0 && last_millis <= 1000)
            delay(last_millis);
        else
            delay(1);
        if (get_vid_stop())
