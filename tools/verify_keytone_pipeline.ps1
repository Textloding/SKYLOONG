...
Assert-Contains $i2sHeader 'void\s+playWAV\(const\s+uint8_t\s+\*data,\s*size_t\s+len\)' "I2S playWAV must accept const WAV buffers."
...
Assert-Contains $i2sCpp 'void\s+I2SClass::playWAV\(const\s+uint8_t\s+\*data,\s*size_t\s+len\)' "I2S playWAV implementation must accept const WAV buffers."
