...
Assert-Contains $web 'VIDEO_TARGET_W\s*=\s*240' "4K uploads must be transcoded to a lower pixel-count playback size."
Assert-Contains $web 'VIDEO_TARGET_H\s*=\s*180' "4K uploads must be transcoded to a lower pixel-count playback size."
Assert-Contains $web 'VIDEO_TARGET_FPS\s*=\s*24' "Video must use an MPEG1-compatible frame rate for firmware playback."
Assert-Contains $web 'VIDEO_BITRATE\s*=\s*"260k"' "Video bitrate must be low enough for ESP32-S3 software playback."
Assert-Contains $web 'VIDEO_MAXRATE\s*=\s*"360k"' "Video maxrate must be capped to avoid playback spikes."
...
Assert-NotContains $web 'scale=320:240' "Full 320x240 transcode is too heavy for smooth 4K-source playback."
