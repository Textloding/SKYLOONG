...
Assert-Contains $managerHeader 'case\s+8:\s*nextApp\s*=\s*appQRCode' "Switching after the pet app must show the QR/Wi-Fi setup screen so first-time Wi-Fi provisioning remains reachable."
Assert-Contains $managerHeader 'case\s+100:\s*nextApp\s*=\s*appHome' "Switching after the QR/Wi-Fi setup screen must return to Home."
Assert-Contains $appManager 'app->appid\s*!=\s*50\s*&&\s*app->appid\s*!=\s*100' "Settings and QR/Wi-Fi setup screens must not be persisted as the next boot app."
