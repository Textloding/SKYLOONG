...
1. 打开心知天气 API V3 文档并登录控制台，创建免费产品。
2. 复制产品的公钥和私钥，分别填入管理台“心知天气”的`公钥`和`私钥`字段。
3. `接口地址`保持默认的 `https://api.seniverse.com`。固件使用 HMAC-SHA1 生成 `ts`、`ttl`、`uid`、`sig` 签名参数，凭据会按天气源独立保存。
4. 免费用户固定请求简体中文和摄氏度，使用实时天气、最多三日预报与生活指数接口；当前界面显示穿衣、运动、感冒、洗车四项建议。
5. 已填写纬度和经度时会优先用 `纬度:经度` 定位，以避免部分中文城市名被接口拒绝；否则使用城市名。
