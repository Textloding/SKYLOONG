...
Assert-Contains $pomodoroCpp 'last_render_progress' "Pomodoro arc rendering must track high precision progress independently from whole-second text."
Assert-Contains $pomodoroCpp 'lv_arc_set_range\(arc_timer,\s*0,\s*10000\)' "Pomodoro arc must use a 0-10000 range so long timers progress smoothly."
Assert-Contains $pomodoroCpp 'lv_obj_set_size\(arc_timer,\s*184,\s*184\)' "Pomodoro arc must be large enough for the 320x240 screen."
Assert-Contains $pomodoroCpp 'lv_font_montserrat_42' "Pomodoro time text must use a slightly smaller font to avoid overlapping the header."
Assert-Contains $pomodoroCpp '\*\s*10000\s*/\s*duration_ms' "Pomodoro arc progress must be derived from remaining milliseconds, not only whole seconds."
...
Assert-Contains $halCpp 'reset_pomodoro_rounds' "Web server must provide a Pomodoro round reset endpoint."
Assert-Contains $pomodoroCpp 'pomodoro_reset_current' "Pomodoro must expose a current phase reset function."
Assert-Contains $pomodoroCpp 'pomodoro_reset_rounds' "Pomodoro must expose a round reset function."
Assert-Contains $pomodoroCpp 'completed_focus_rounds\s*=\s*0' "Pomodoro round reset must return to round 1."
...
Assert-Contains $web 'data-reset-pomodoro-rounds' "Web UI must let desktop users reset Pomodoro rounds."
Assert-Contains $web 'reset_pomodoro_rounds' "Web UI must call the Pomodoro round reset endpoint."
...
Assert-Contains $mock 'reset_pomodoro_rounds' "Mock server must handle Pomodoro round reset requests."
